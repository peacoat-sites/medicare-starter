---
title: "Medicare Cataract Surgery Coverage: What You'll Pay"
date: 2026-08-03T16:35:12.215321+00:00
draft: false
description: "Learn what Medicare covers for cataract surgery, including costs, copays, and out-of-pocket expenses you may face."
image: "/img/heroes/12104186.jpg"
categories: ["Does Medicare Cover"]
tags: ["does", "medicare", "cover", "cataract", "surgery"]
author: "Susan Park"
...
faqs:
  - q: "Does Medicare cover cataract surgery for both eyes?"
    a: "Yes. Medicare Part B covers cataract surgery on each eye as a separate medically necessary procedure. You'll go through the same cost-sharing process (deductible, then 20% coinsurance) for each surgery, though if both happen in the same calendar year you'll only have one annual deductible to meet."
  - q: "Will Medicare pay for laser cataract surgery?"
    a: "Laser-assisted cataract surgery (FLACS) uses a femtosecond laser for certain steps of the procedure. Medicare covers the medically necessary portions of the surgery, but the additional cost of the laser technology above the standard surgical technique is generally billed as an upgrade and falls to the patient. This is similar to the premium lens situation: ask your surgeon exactly what portion is billed to Medicare and what's an out-of-pocket upgrade charge."
  - q: "What if my cataract isn't 'bad enough' yet? Will Medicare still cover it?"
    a: "Medicare covers cataract surgery when it's medically necessary, meaning the cataract is significantly impairing your vision and daily function. A visual acuity standard or documented functional impairment is typically required for Medicare to approve coverage. Your ophthalmologist documents this in your medical record. If your cataract is early-stage and not meaningfully affecting your vision, Medicare may not cover surgery at that point. Your doctor will tell you when you've reached that threshold."
  - q: "Can I use my Medicare Advantage plan instead of Original Medicare for cataract surgery?"
    a: "Yes, and Medicare Advantage (Part C) plans must cover everything Original Medicare covers. However, you'll need to follow your plan's rules: use in-network providers, get referrals if your plan requires them, and confirm prior authorization isn't needed. Copays and cost-sharing vary widely by plan, so call your plan's member services before scheduling."
  - q: "Does Medicare cover the eye exam to diagnose cataracts?"
    a: "This is one people get wrong all the time, myself included the first time I looked it up. Routine eye exams are not covered by Original Medicare Part B. However, a medical eye exam with an ophthalmologist to diagnose and evaluate a cataract is covered as a medical service, not a routine vision exam. The distinction is whether the exam is for a medical condition vs. a general vision check. If your primary care doctor refers you to an ophthalmologist because of vision changes or a suspected cataract, that visit is generally covered under Part B."
---
...
<style>.stat-chart{margin:28px 0;padding:18px 20px;border:1px solid var(--border,#e7e5e4);border-left:4px solid var(--accent,#4338ca);border-radius:12px;background:var(--surface2,#f8fafc)}.stat-chart .sc-title{font-weight:700;margin-bottom:12px;color:var(--heading,#1e293b)}.stat-chart .sc-row{display:flex;align-items:center;gap:10px;margin:7px 0}.stat-chart .sc-label{flex:0 0 34%;font-size:.85rem;color:var(--muted,#475569);text-align:right;overflow-wrap:anywhere}.stat-chart .sc-track{flex:1;background:var(--border,#e7e5e4);border-radius:6px;height:14px;overflow:hidden}.stat-chart .sc-bar{display:block;height:100%;background:var(--accent,#4338ca);border-radius:6px}.stat-chart .sc-val{flex:0 0 auto;font-size:.82rem;font-weight:600;color:var(--heading,#1e293b);min-width:56px}.stat-chart .sc-src{margin-top:10px;font-size:.75rem;color:var(--muted,#64748b)}@media(max-width:560px){.stat-chart .sc-label{flex-basis:42%}}</style><div class="stat-chart"><div class="sc-title">Estimated out-of-pocket cost per eye by coverage type</div><div class="sc-row"><span class="sc-label">Part B only (no Medigap)</span><span class="sc-track"><span class="sc-bar" style="width:100%"></span></span><span class="sc-val">$320</span></div><div class="sc-row"><span class="sc-label">Part B + Medigap Plan G</span><span class="sc-track"><span class="sc-bar" style="width:9%"></span></span><span class="sc-val">$30</span></div><div class="sc-row"><span class="sc-label">Part B + Medigap Plan N</span><span class="sc-track"><span class="sc-bar" style="width:23%"></span></span><span class="sc-val">$75</span></div><div class="sc-row"><span class="sc-label">Medicare Advantage avg copay</span><span class="sc-track"><span class="sc-bar" style="width:86%"></span></span><span class="sc-val">$275</span></div><div class="sc-src">Source: CMS physician fee schedule estimates, 2026</div></div>
...
1. Confirm your surgeon is a Medicare-participating provider (not just "Medicare-accepting." Participating means they've agreed to accept Medicare's approved amount as payment in full, which protects you from excess charges in most states).
2. Ask the facility whether it's classified as a hospital outpatient department or an ambulatory surgical center. The cost-sharing is different under Medicare's rules.
3. Call your Medigap or Medicare Advantage plan and confirm the surgery is covered before the date. Get a reference number for that call.
4. Ask the surgeon's billing team to give you an itemized cost estimate in writing, including any lens upgrade charges, before you sign the consent form.
...
- [Medicare.gov](https://www.medicare.gov/): Official Medicare coverage details for cataract surgery and eyewear benefits, current as of 2026
- [Centers for Medicare & Medicaid Services (CMS)](https://www.cms.gov/): Physician Fee Schedule and Outpatient Prospective Payment System data
